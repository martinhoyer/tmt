...
def test_operator_to_beaker_op(
    operator: Operator, value: str, expected: tuple[str, str, bool]
) -> None:
...
def test_disk_driver(root_logger: Logger) -> None:
    ...
    assert result.to_mrack() == {
        'key_value': {'_key': 'BOOTDISK', '_op': '==', '_value': 'mpt3sas'}
    }
    ...
    assert result.to_mrack() == {
        'key_value': {'_key': 'BOOTDISK', '_op': 'like', '_value': 'mpt3%'}
    }
    ...
    assert result.to_mrack() == {
        'not': {'key_value': {'_key': 'BOOTDISK', '_op': 'like', '_value': 'mpt3%'}}
    }
...
def test_virtualization_hypervisor(root_logger: Logger) -> None:
    ...
    assert result.to_mrack() == {
        'not': {'system': {'hypervisor': {'_op': 'like', '_value': 'kvm'}}}
    }
...
def test_zcrypt_adapter(root_logger: Logger) -> None:
    ...
    assert result.to_mrack() == {
        'system': {'key_value': {'_key': 'ZCRYPT_MODEL', '_op': 'like', '_value': 'CEX%'}}
    }
    ...
    assert result.to_mrack() == {
        'not': {'system': {'key_value': {'_key': 'ZCRYPT_MODEL', '_op': 'like', '_value': 'CEX%'}}}
    }
...
def test_zcrypt_mode(root_logger: Logger) -> None:
    ...
    assert result.to_mrack() == {
        'not': {'system': {'key_value': {'_key': 'ZCRYPT_MODE', '_op': 'like', '_value': 'C%A'}}}
    }
...
def test_iommu_is_supported(root_logger: Logger) -> None:
    for value in True, False:
        ...
        assert result.to_mrack() == {
            'key_value': {'_key': 'VIRT_IOMMU', '_op': '==', '_value': str(int(value))}
        }
...
def test_system_model_name(root_logger: Logger) -> None:
    ...
    assert result.to_mrack() == {
        'not': {'system': {'model': {'_op': 'like', '_value': 'PowerEdge R750%'}}}
    }
