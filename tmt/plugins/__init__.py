...
def _import(
    *,
    ...
    Args:
        module: name of a module to import. It may represent a submodule
            as well, using common dot notation (``foo.bar.baz``).

    Returns:
        imported module.

    Raises:
        tmt.utils.GeneralError: when import fails.
    """
...
def _import_or_raise(
    *,
    ...
    Args:
        module: name of a module to import. It may represent a submodule
            as well, using common dot notation (``foo.bar.baz``).
        exc_class: an exception class to raise on failure.
        exc_message: an exception message.
...
def import_module(
    *,
    ...
    Args:
        module: name of a module to import. It may represent a submodule
            as well, using common dot notation (``foo.bar.baz``).
        path: if specified, it would be incorporated in exception
            message.
...
class PluginRegistry(Generic[RegisterableT]):
    ...
    def register_plugin(
        ...
        *,
        ...
        Args:
            plugin_id: id of the plugin. Works as a label or name, and
                must may not be used in this registry yet.
            plugin: a plugin to register.
            raise_on_conflict: if set, an exception would be raised when
                id was already used.
