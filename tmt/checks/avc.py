...
# Common template for the backlog checking function using auditctl -s (report)
# "backlog field tells how many event records are currently queued waiting for auditd to read them"
BACKLOG_CHECK_FUNCTION = """
wait_for_backlog() {
    max_wait=10
    poll_interval=0.5
    waited=0
    ...
    while [ $waited -lt $max_wait ]; do
        backlog=$(auditctl -s 2>/dev/null | grep -E 'backlog *[0-9]+' | awk '{print $2}')
...
{{% else %}}
ausearch --input-logs --checkpoint {{{{ MARK_FILEPATH }}}} --just-one
{{% endif %}}
""")
)
...
{{% else %}}
ausearch --input-logs --checkpoint {{{{ MARK_FILEPATH }}}} -m AVC -m USER_AVC -m SELINUX_ERR -i
{{% endif %}}
""")
)
