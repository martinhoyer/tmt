...
JUNIT_XML_FILENAME = "junit-report.xml"
    ...
    def extract_results(
        ...
        if junit_xml_filepath.exists():
            try:
                tree = ET.parse(junit_xml_filepath)
                root = tree.getroot()
                ...
                testsuite_summary = root.find('.') # The root element is <testsuites> or <testsuite>
                ...
                for testcase_node in root.findall('.//testcase'):
                    ...
                    if failure_node is not None:
                        ...
                        if failure_node.text and (not message or len(failure_node.text) > len(message)):
                             message = f"{message}: {failure_node.text.strip()}" if message else failure_node.text.strip()
                    ...
                    if error_node is not None:
                        ...
                        if error_node.text and (not message or len(error_node.text) > len(message)):
                             message = f"{message}: {error_node.text.strip()}" if message else error_node.text.strip()


                    # Update overall_outcome (worst of current vs new)
                    if current_outcome == ResultOutcome.ERROR:
                        overall_outcome = ResultOutcome.ERROR
                    elif current_outcome == ResultOutcome.FAIL and overall_outcome != ResultOutcome.ERROR:
                        overall_outcome = ResultOutcome.FAIL
                    elif current_outcome == ResultOutcome.SKIP and overall_outcome == ResultOutcome.PASS:
                    ...
                    sub_results.append(tmt.result.SubResult(
                        name=result_name,
                ...
                # Handle case where <testsuite> might be empty (e.g. pytest --collect-only with no tests)
                # or if all tests were skipped resulting in non-zero exit code (e.g. pytest.exit("all skipped", 5))
        ...
        if invocation.return_code == 0:
            ...
            if invocation.return_code not in (1, 5):
                 note_list.append(f"Pytest exited with an unexpected code: {invocation.return_code}")
            elif invocation.return_code == 1 and not junit_xml_filepath.exists():
                 note_list.append(f"Pytest failed (exit code 1) and JUnit XML report was not found.")
